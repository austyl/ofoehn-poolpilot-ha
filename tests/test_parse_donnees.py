...
parse_donnees = coordinator.parse_donnees


def test_parse_donnees_parses_negative_values():
    raw = "DONNEE1=-12.3"
    result = parse_donnees(raw)
    assert result[1] == -12.3


def test_parse_donnees_parses_comma_values():
    raw = "DONNEE2=45,6"
    result = parse_donnees(raw)
    assert result[2] == 45.6
